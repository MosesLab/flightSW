...
int exitDefault() {
    ...
    char ack;
    if (receiveAck(roe.roeLink, &ack, 1, 0x03) == -1) {
...
int getHK(char hkparam) {
    ...
    char response;
    if (receiveAck(roe.roeLink, (char *) &response, 1, ROE_HK_RES) == -1) {
        pthread_mutex_unlock(&roe.mx);
...
int reset() {
    ...
    if (write(roe.roeLink, (char *) &command, 1) != 1) //Write Command to ROE Link
    {
        record("Reset Error\n");
        pthread_mutex_unlock(&roe.mx);
        return -1;
...
int selftestMode() {
    ...
    for (i = 0; i < 9; i++) //Write the command to the serial link
        if (write(roe.roeLink, (char*) &selftst[i], 1) != 1)
        {
...
int stimOn() {
    ...
    int i;
    for (i = 0; i < 9; i++) //Write the command to the serial link
        if (write(roe.roeLink, (char*) &stimon[i], 1) != 1)
        {
...
int stimOff() {
    ...
    int i;
    for (i = 0; i < 9; i++) //Write the command to the serial link
        if (write(roe.roeLink, (char*) &stimoff[i], 1) != 1)
        {
...
int flush() {
    ...
    for (i = 0; (status != END_OF_SEQUENCE)&&(i < 5); i++) {
        readRoe(roe.roeLink, &status, 1); //Wait for endofsequence, faster than usleep(***)
    }
    readRoe(roe.roeLink, &status, 1); //Get the status of command execution
